...
export function CustomerSuccessWorkspace() {
  ...
  if (!data) return <Panel className="p-6 text-sm text-slate-500">{error || "Loading customer success…"}</Panel>;
...
function Metric({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) { return <Panel className="p-5"><div className="flex items-center gap-2 text-sm text-slate-500">{icon}{label}</div><div className="mt-2 text-3xl font-semibold">{value}</div></Panel>; }
...
function PlanDialog({ plan, companies, account, busy, close, save }: { plan?: Row; companies: Row[]; account: string; busy: boolean; close: () => void; save: (form: FormData) => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 p-4" onClick={close}><Panel className="max-h-[94vh] w-full max-w-3xl overflow-y-auto p-6"><form className="grid gap-4" onClick={event => event.stopPropagation()} onSubmit={event => { event.preventDefault(); void save(new FormData(event.currentTarget)); }}>
