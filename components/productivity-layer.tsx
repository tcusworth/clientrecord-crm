 ...
 return <><DraftRecovery/><UndoCenter/>
  ...
  <CommandDialog open={open} onOpenChange={setOpen} title="ClientRecord command bar" description="Find records, navigate, or run a CRM action." className="w-[calc(100vw-2rem)] max-w-2xl">
   ...
   <CommandList><CommandEmpty>No matching records or commands.</CommandEmpty>
    ...
    <CommandSeparator/>
    {recent.length>0&&<CommandGroup heading="Recent">{recent.map(item=><CommandItem key={`${item.kind}:${item.id}`} value={`Recent ${item.label}`} onSelect={()=>{if(item.kind==="contact"){const contact=contacts.find(value=>String(value.id)===item.id);if(contact){setOpen(false);onContact(contact)}}else go(item.id,item.label)}}><RotateCcw/><span>{item.label}</span></CommandItem>)}</CommandGroup>}
    ...
    <CommandGroup heading="Companies">{companies.slice(0,50).map(company=><CommandItem key={company.id} value={`${company.name} ${company.stage}`} onSelect={()=>go("companies",company.name)}><Building2/><span>{company.name}</span><span className="ml-auto text-xs text-slate-500">{company.stage}</span></CommandItem>)}</CommandGroup>
