...
export const inboxMessages = sqliteTable("inbox_messages", {
  id: text("id").primaryKey(), provider: text("provider").notNull().default("Manual"), externalId: text("external_id"), direction: text("direction").notNull().default("Inbound"), fromEmail: text("from_email").notNull(), fromName: text("from_name").notNull().default(""), toEmails: text("to_emails").notNull().default("[]"), subject: text("subject").notNull().default(""), body: text("body").notNull().default(""), attachmentsJson: text("attachments_json").notNull().default("[]"), status: text("status").notNull().default("Unassigned"), owner: text("owner").notNull().default(""), contactId: integer("contact_id").references(()=>contacts.id), companyId: integer("company_id").references(()=>companies.id), dealId: integer("deal_id").references(()=>deals.id), taskId: integer("task_id").references(()=>tasks.id), occurredAt: text("occurred_at").notNull(), createdAt: text("created_at").notNull(), updatedAt: text("updated_at").notNull(),
}, t => [uniqueIndex("inbox_messages_provider_external").on(t.provider,t.externalId),index("inbox_messages_status_date").on(t.status,t.occurredAt),index("inbox_messages_contact").on(t.contactId)]);
