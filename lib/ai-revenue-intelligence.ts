...
// Forecast dollar totals are always computed from deal records, never taken from a model.
export function forecastTotals(deals:Row[]){
  const open=deals.filter(item=>clean(item.status,50)==="Open"),byCategory=(category:string)=>open.filter(item=>clean(item.forecast_category,80).toLowerCase()===category).reduce((sum,item)=>sum+money(item.value),0);
  return {forecastTotal:Math.round(open.reduce((sum,item)=>sum+money(item.value)*Number(item.probability||0)/100,0)),commitTotal:byCategory("commit"),bestCaseTotal:byCategory("best case"),pipelineTotal:byCategory("pipeline")};
}

export function buildForecastExplanation(input:{deals:Row[];stageHistory:Row[]}){
  const open=input.deals.filter(item=>clean(item.status,50)==="Open"),commit=open.filter(item=>clean(item.forecast_category,80).toLowerCase()==="commit"),recentMoves=input.stageHistory.filter(item=>{const age=daysAgo(item.happened_at);return age!==null&&age<=7});
...
const totalKeys:string[]=["forecastTotal","commitTotal","bestCaseTotal","pipelineTotal"];
// Model-facing schema: narrative and judgment only; totals are filled in from forecastTotals().
export const forecastNarrativeSchema={...forecastExplanationSchema,properties:Object.fromEntries(Object.entries(forecastExplanationSchema.properties).filter(([key])=>!totalKeys.includes(key))),required:forecastExplanationSchema.required.filter(key=>!totalKeys.includes(key))};
