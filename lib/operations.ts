...
export async function runInboxSlaAlerts() {
  const overdue=(await env.DB.prepare("SELECT m.id,m.owner,m.subject,m.deal_id AS dealId FROM inbox_messages m WHERE m.direction='Inbound' AND m.status NOT IN ('Resolved') AND m.response_due_at IS NOT NULL AND datetime(m.response_due_at)<datetime('now')").all<Row>()).results;let created=0;
