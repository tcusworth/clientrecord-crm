...
async function prepareMeetingBrief(dealId:number,meetingKey:string,userEmail:string){
  ...
  const inputHash=await sha256(JSON.stringify(sourceGroups.map(source=>({type:source.type,id:source.id,updatedAt:source.updatedAt,value:source.value})))),entityId=`${dealId}:${meetingKey}`,cached=await one("SELECT a.*,(SELECT count(*) FROM ai_artifact_sources s WHERE s.artifact_id=a.id) AS source_count FROM ai_artifacts a WHERE a.feature='meeting-prep' AND a.entity_type='meeting' AND a.entity_id=? AND a.input_hash=? AND a.sensitive=0 AND a.review_status!='Rejected' ORDER BY a.generated_at DESC LIMIT 1",entityId,inputHash);
...
export async function GET(request:Request){
  ...
  try{
    ...
    const [company,notes,activities,tasks,history,stakeholders,lineItems,insights,reviews,proposals,documents,unassigned,meetings,meetingAttendees,briefRows]=await Promise.all([
      ...
      rows("SELECT m.*,CASE WHEN ?=1 OR d.sensitive=0 THEN d.title END AS transcript_title FROM deal_meetings m LEFT JOIN client_documents d ON d.id=m.transcript_document_id WHERE m.deal_id=? ORDER BY m.starts_at DESC",canSeeSensitive(user)?1:0,dealId),
      ...
      rows(`SELECT a.*,(SELECT count(*) FROM ai_artifact_sources s WHERE s.artifact_id=a.id) AS source_count FROM ai_artifacts a WHERE a.feature='meeting-prep' AND a.entity_type='meeting' AND a.entity_id LIKE ? AND a.superseded_by IS NULL AND a.review_status!='Rejected' AND ${visibleArtifactSql(user)} ORDER BY a.generated_at DESC`,`${dealId}:%`),
    ]);
    const health=calculateHealth(deal,tasks,activities,stakeholders,insights,reviews,lineItems),relationship=await relationshipHealth(dealId,deal,tasks,activities,stakeholders),coverage=calculateStakeholderCoverage({deal,stage:await coverageStage(deal),stakeholders,activities}),nextAction=await nextBestAction(dealId,deal,tasks,reviews,proposals,insights,activities,coverage);
    const timeline=[
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(action==="saveNote"){
      ...
      const transcriptDocumentId=text(body.transcriptDocumentId,100)||null,transcript=transcriptDocumentId?await one("SELECT id,sensitive FROM client_documents WHERE id=? AND deal_id=? AND status='Active'",transcriptDocumentId,dealId):null;if(transcriptDocumentId&&!transcript)throw new Error("Choose a transcript document associated with this deal.");
      // Linking a sensitive document as a transcript needs documents.manage_sensitive; re-saving an existing link is allowed.
      if(transcript?.sensitive&&!canSeeSensitive(user)&&before?.transcript_document_id!==transcriptDocumentId)return Response.json({error:"Sensitive-document permission is required to link this transcript."},{status:403});
