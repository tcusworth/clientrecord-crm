...
async function customFieldChanges(body: Record<string, unknown>, entityType: "contact" | "company", entityId: number) {
  ...
  for (const definition of definitions) {
    ...
    if (value && fieldType === "select") {
      const options = JSON.parse(String(definition.options || "[]")) as string[];
      if (!options.includes(value)) throw new Error("Choose a configured option for each select field.");
    }
    statements.push(env.DB.prepare("DELETE FROM custom_field_values WHERE definition_id=? AND entity_type=? AND entity_id=?").bind(id, entityType, entityId));
    if (value) statements.push(env.DB.prepare("INSERT INTO custom_field_values(definition_id,entity_type,entity_id,value,updated_at) VALUES (?,?,?,?,datetime('now'))").bind(id, entityType, entityId, value));
  }
  return statements;
}
...
export async function POST(request: Request) {
  try {
    ...
    // mergeContacts deletes the merged contact, so it needs records.delete as well as records.edit.
    ...
    if(body.action==="mergeContacts"){
      ...
      const tags=Array.from(new Set([...JSON.parse(String(keep.tags||"[]")),...JSON.parse(String(merge.tags||"[]"))])),keepKey=String(keepId),mergeKey=String(mergeId);await env.DB.batch([...CONTACT_REFERENCES.map(([table,column])=>env.DB.prepare(`UPDATE ${table} SET ${column}=? WHERE ${column}=?`).bind(keepId,mergeId)),env.DB.prepare("UPDATE contacts SET company=?,title=?,phone=?,location=?,notes=?,stage=?,tags=?,last_contact=COALESCE(last_contact,?),next_follow_up=COALESCE(next_follow_up,?),subscribed=?,updated_at=datetime('now') WHERE id=?").bind(clean(keep.company)||clean(merge.company),clean(keep.title)||clean(merge.title),clean(keep.phone)||clean(merge.phone),clean(keep.location)||clean(merge.location),[clean(keep.notes),clean(merge.notes)].filter(Boolean).join("\n\n"),clean(keep.stage,"Lead"),JSON.stringify(tags),merge.last_contact||null,merge.next_follow_up||null,Boolean(keep.subscribed)&&Boolean(merge.subscribed)?1:0,keepId),env.DB.prepare("INSERT INTO account_stakeholders(company_id,contact_id,role,notes) SELECT company_id,?,role,notes FROM account_stakeholders WHERE contact_id=? ON CONFLICT(company_id,contact_id) DO UPDATE SET notes=account_stakeholders.notes||char(10)||'Merged relationship: '||excluded.role||' '||excluded.notes").bind(keepId,mergeId),env.DB.prepare("DELETE FROM account_stakeholders WHERE contact_id=?").bind(mergeId),env.DB.prepare("INSERT INTO deal_stakeholders(deal_id,contact_id,role,notes,is_primary,active,created_at,updated_at) SELECT deal_id,?,role,notes,is_primary,active,created_at,datetime('now') FROM deal_stakeholders WHERE contact_id=? ON CONFLICT(deal_id,contact_id) DO UPDATE SET notes=deal_stakeholders.notes||char(10)||'Merged relationship: '||excluded.role||' '||excluded.notes,is_primary=max(deal_stakeholders.is_primary,excluded.is_primary),active=max(deal_stakeholders.active,excluded.active),updated_at=excluded.updated_at").bind(keepId,mergeId),env.DB.prepare("DELETE FROM deal_stakeholders WHERE contact_id=?").bind(mergeId),env.DB.prepare("INSERT INTO custom_field_values(definition_id,entity_type,entity_id,value,updated_at) SELECT source.definition_id,'contact',?,source.value,source.updated_at FROM custom_field_values source WHERE source.entity_type='contact' AND source.entity_id=? AND NOT EXISTS(SELECT 1 FROM custom_field_values existing WHERE existing.definition_id=source.definition_id AND existing.entity_type='contact' AND existing.entity_id=?)").bind(keepId,mergeId,keepId),env.DB.prepare("DELETE FROM custom_field_values WHERE entity_type='contact' AND entity_id=?").bind(mergeId),
        // Polymorphic references: the kept contact's own values win on unique conflicts; history (audit/import/undo logs, AI runs/artifacts) keeps the old id.
        env.DB.prepare("UPDATE OR IGNORE ai_record_fields SET entity_id=? WHERE entity_type='contact' AND entity_id=?").bind(keepKey,mergeKey),env.DB.prepare("DELETE FROM ai_record_fields WHERE entity_type='contact' AND entity_id=?").bind(mergeKey),env.DB.prepare("UPDATE notifications SET entity_id=? WHERE entity_type='contact' AND entity_id=?").bind(keepKey,mergeKey),env.DB.prepare("UPDATE OR IGNORE custom_relationships SET from_entity_id=? WHERE from_entity_type='contact' AND from_entity_id=?").bind(keepKey,mergeKey),env.DB.prepare("UPDATE OR IGNORE custom_relationships SET to_entity_id=? WHERE to_entity_type='contact' AND to_entity_id=?").bind(keepKey,mergeKey),env.DB.prepare("DELETE FROM custom_relationships WHERE (from_entity_type='contact' AND from_entity_id=?) OR (to_entity_type='contact' AND to_entity_id=?)").bind(mergeKey,mergeKey),env.DB.prepare("DELETE FROM contacts WHERE id=?").bind(mergeId)]);return Response.json({status:"merged",id:keepId});
    }
