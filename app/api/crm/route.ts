...
export async function POST(request: Request) {
  try {
    ...
    if(body.action==="mergeContacts"){
      ...
      const tags=Array.from(new Set([...JSON.parse(String(keep.tags||"[]")),...JSON.parse(String(merge.tags||"[]"))]));await env.DB.batch([env.DB.prepare("UPDATE activities SET contact_id=? WHERE contact_id=?").bind(keepId,mergeId),env.DB.prepare("UPDATE tasks SET contact_id=? WHERE contact_id=?").bind(keepId,mergeId),env.DB.prepare("UPDATE companies SET primary_contact_id=? WHERE primary_contact_id=?").bind(keepId,mergeId),env.DB.prepare("UPDATE contacts SET company=?,title=?,phone=?,location=?,notes=?,stage=?,tags=?,last_contact=COALESCE(last_contact,?),next_follow_up=COALESCE(next_follow_up,?),subscribed=?,updated_at=datetime('now') WHERE id=?").bind(clean(keep.company)||clean(merge.company),clean(keep.title)||clean(merge.title),clean(keep.phone)||clean(merge.phone),clean(keep.location)||clean(merge.location),[clean(keep.notes),clean(merge.notes)].filter(Boolean).join("\n\n"),clean(keep.stage,"Lead"),JSON.stringify(tags),merge.last_contact||null,merge.next_follow_up||null,Boolean(keep.subscribed)&&Boolean(merge.subscribed)?1:0,keepId),env.DB.prepare("UPDATE deals SET contact_id=? WHERE contact_id=?").bind(keepId,mergeId),env.DB.prepare("UPDATE automation_enrollments SET contact_id=? WHERE contact_id=?").bind(keepId,mergeId),env.DB.prepare("UPDATE sync_records SET contact_id=? WHERE contact_id=?").bind(keepId,mergeId),env.DB.prepare("INSERT INTO account_stakeholders(company_id,contact_id,role,notes) SELECT company_id,?,role,notes FROM account_stakeholders WHERE contact_id=? ON CONFLICT(company_id,contact_id) DO UPDATE SET notes=account_stakeholders.notes||char(10)||'Merged relationship: '||excluded.role||' '||excluded.notes").bind(keepId,mergeId),env.DB.prepare("DELETE FROM account_stakeholders WHERE contact_id=?").bind(mergeId),env.DB.prepare("INSERT INTO custom_field_values(definition_id,entity_type,entity_id,value,updated_at) SELECT source.definition_id,'contact',?,source.value,source.updated_at FROM custom_field_values source WHERE source.entity_type='contact' AND source.entity_id=? AND NOT EXISTS(SELECT 1 FROM custom_field_values existing WHERE existing.definition_id=source.definition_id AND existing.entity_type='contact' AND existing.entity_id=?)").bind(keepId,mergeId,keepId),env.DB.prepare("DELETE FROM custom_field_values WHERE entity_type='contact' AND entity_id=?").bind(mergeId),env.DB.prepare("DELETE FROM contacts WHERE id=?").bind(mergeId)]);return Response.json({status:"merged",id:keepId});
    }
