...
export async function POST(request: Request) {
  ...
  if (!event.type?.startsWith("email.")) return Response.json({ accepted: true });
  ...
  if (recipient && (event.type === "email.bounced" || event.type === "email.complained" || event.type === "email.suppressed")) {
    await env.DB.prepare("INSERT INTO delivery_logs(provider,kind,status,recipient,external_id,error,context,created_at) VALUES ('resend','webhook','Failed',?,?,?,?,datetime('now'))")
      .bind(recipient,event.data?.email_id||null,event.type.replace("email.",""),payload.slice(0,32000)).run();
  }
  if (!broadcastId) return Response.json({ accepted: true });
