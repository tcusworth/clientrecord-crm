...
export async function POST(request:Request){
 ...
 try{const body=await request.json() as Row,action=clean(body.action,80),now=new Date().toISOString();
  ...
  if(action==="routeLead"){
    ...
    const proposalId=id(body.id),status=clean(body.status,30);if(!["Draft","Sent","Opened","Accepted","Declined","Expired"].includes(status))throw new Error("Choose a valid proposal status.");const row=(await rows("SELECT * FROM deal_proposals WHERE id=?",proposalId))[0];if(!row)throw new Error("Proposal not found.");await env.DB.prepare("UPDATE deal_proposals SET status=?,sent_at=CASE WHEN ?='Sent' THEN COALESCE(sent_at,?) ELSE sent_at END,opened_at=CASE WHEN ?='Opened' THEN COALESCE(opened_at,?) ELSE opened_at END,accepted_at=CASE WHEN ?='Accepted' THEN COALESCE(accepted_at,?) ELSE accepted_at END,accepted_by=CASE WHEN ?='Accepted' THEN ? ELSE accepted_by END,updated_at=? WHERE id=?").bind(status,status,now,status,now,status,now,status,clean(body.acceptedBy,200)||user.email,now,proposalId).run();await audit(user,"proposal.status","deal_proposal",proposalId,`Marked proposal ${status.toLowerCase()}`,{status});
